...
): { previous: Goal | null; next: Goal | null } {
...
async function createGoal(
  ...
  if (!initialized) {
    ...
    const hierarchy = await createInitialHierarchy(
      params.goal.title,
  ...
  const order =
    params.goal.order ??
    (parentId ? hierarchy.goals[parentId].childIds.length : 0);
  ...
  if (params.filter) {
    if (params.filter.status) {
      filteredGoals = filteredGoals.filter(
        (g) => g.status === params.filter!.status,
      );
    }

    if (params.filter.parentId !== undefined) {
      filteredGoals = filteredGoals.filter(
        (g) => g.parentId === params.filter!.parentId,
      );
    }
    ...
    if (params.filter.includeChildren) {
      ...
      filteredGoals = Array.from(goalsWithChildren).map(
        (id) => hierarchy.goals[id],
      );
    }
  }
...
async function updateGoal(
  ...
  if (params.goal) {
    ...
    if (params.goal.dependencies !== undefined)
      goal.dependencies = params.goal.dependencies;
  }
 ...
 * Main tool handler
 */
export async function handleGoalManagement(
  params: unknown,
): Promise<GoalManagementResult> {
  try {
...
export const goalManagementTool = {
  ...
  description:
    'Manage hierarchical goals and track progress. Supports create, read, update, delete, complete, advance, get-context, set-current, analyze-dependencies, calculate-priorities, and reorder actions. Includes dependency graph analysis and automatic priority calculation.',
